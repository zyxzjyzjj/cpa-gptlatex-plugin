...
func handlePanel(request []byte) ([]byte, error) {
	...
	if req.Method == http.MethodGet && pasted == "" {
		// The authorization URL is generated by the provider flow (auth.login.start
		// runs it), so the page only has to show it -- clicking the link here opens
		// it in a normal tab, which is what keeps the callback copyable.
		authorizeURL := ""
		if flow := pendingLogin(); flow != nil {
			authorizeURL = flow.authorizeURL
		}
		return htmlReply(http.StatusOK, panelHTML("", "", false, authorizeURL)), nil
	}
	if pasted == "" {
...
func loginLinkHTML(authorizeURL string) string {
	if strings.TrimSpace(authorizeURL) == "" {
		return `<p class="sub"><b>先在 CPA 里点一次登录</b>（生成授权 URL 的那一步），然后刷新本页，` +
			`这里就会出现可以正常打开的登录链接。</p>`
	}
	escaped := strings.NewReplacer("&", "&amp;", "<", "&lt;", ">", "&gt;", `"`, "&quot;").Replace(authorizeURL)
	return `<p class="sub">① 在新标签页里打开这个登录链接（<b>不要用 CPA 的按钮</b>，那会开一个授权后自关的弹窗）：</p>
